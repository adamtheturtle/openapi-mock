...
def _run_respx(spec: dict, url: str, method: str = "GET", params: dict | None = None) -> tuple[int, object]:
...
def _run_responses(spec: dict, url: str, method: str = "GET", params: dict | None = None) -> tuple[int, object]:
...
def _run(backend: str, spec: dict, url: str, method: str = "GET", params: dict | None = None) -> tuple[int, object]:
...
def test_query_params(backend: str) -> None:
    ...
    status, body = _run(backend=backend, spec=spec, url=f"{BASE_URL}/pets", params={"limit": 10})
    assert status == HTTPStatus.OK
    assert body == []
